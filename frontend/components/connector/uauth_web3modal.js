...
let w3m;
export const registerWeb3Modal = (web3modal) => {
  w3m = web3modal;
};
...
export const getUAuth = (UAuth, opts) => {
  return new UAuth(opts);
};

export const connector = async (UAuth, opts) => {
  ...
  if (["web3", "injected"].includes(user.wallet_type_hint)) {
    ...
    provider = connectors.walletconnect(
      w3m.providerController.getProviderOption(id, "package"),
      {
        network: opts.network,
        ...w3m.providerController.getProviderOption(id, "options"),
      }
    );
  } else {
